...
def tic_tac_toe_run(env=None, agent=None, states_to_explain=None):
    ...
    characteristic_results = [local_sverl_characteristics, policy_characteristics, value_characteristics]
    characteristic_names = ["local", "policy", "value_function"]
    return characteristic_results, characteristic_names
...
if __name__ == "__main__":
    ...
    explainer.run(char_list=char_list, names=names)
    explainer.print()

    # for char_data, characteristic_type in zip(char_list, names):
    #     explainer_values = explainer.run(char_data)
    #     results["shapley"][characteristic_type] = explainer_values
