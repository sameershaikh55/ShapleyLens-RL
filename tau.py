...
class TauValue:
    ...
    def _as_value_array(self, value):
        """
        Converts scalar or vector characteristic value to numpy array.
        Allows for characteristic values that are vectors (e.g., for each action) by summing over the vector to get a scalar value, which is necessary for the Tau value calculation. If the characteristic value is already a scalar, it is converted to a numpy array with one element.
        """
    ...
    def _get_value(self, characteristic_values, C, state):
        ...
        Subtract the value of the empty coalition to ensure that v(empty) = 0, which is a requirement for the Tau value calculation.
        """
        C = tuple(sorted(C))
        state = tuple(state)
        ...
        #return (characteristic_values[C][state]-characteristic_values[()][state])
        value = self._as_value_array(characteristic_values[C][state])
        empty_value = self._as_value_array(characteristic_values[()][state])

        return value - empty_value #Subtracting the value of the empty coalition to ensure that v(empty) = 0, which is a requirement for the Tau value calculation.
    ...
    def _compute_vmax(self, characteristic_values, state):
        ...
        vmax = np.zeros((self.F_card,) + grand_value.shape) #array of zeros (array shape of grand value) with length of number of features
    ...
    def _compute_dn(self, characteristic_values, state, feature, vmax):
        ...
        """
        grand_value = self._get_value(characteristic_values, tuple(self.F), state)
        best = np.full_like(grand_value, -np.inf, dtype=float)
    ...
    def _compute_vmin(self, characteristic_values, state, vmax):
        ...
        """
        grand_value = self._get_value(characteristic_values, tuple(self.F), state)
        vmin = np.zeros((self.F_card,) + grand_value.shape) 
    ...
    def run(self, characteristic_values):
        ...
        for state in self.states:
            ...
            vmax = self._compute_vmax(characteristic_values, state)
            vmin = self._compute_vmin(characteristic_values, state, vmax)

            """#vmax, vmin aren't necessarily v1, v2
            vmax = np.zeros(self.F_card)
            vmin = np.zeros(self.F_card)

            for i in self.F:
                vmax[i] = max(v1[i], v2[i])
                vmin[i] = min(v1[i], v2[i])"""

            #self._integrity_check(grand_value, vmax, vmin, state)

            sum_vmax = np.sum(vmax, axis=0)
            sum_vmin = np.sum(vmin, axis=0)

            denominator = sum_vmax - sum_vmin
            ...
            tau_values[state] = [tau[i] for i in self.F]
