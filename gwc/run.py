...
def gwc_get_characteristic_modes():
    return ['local_sverl', 'global_sverl', 'shapley_on_policy', 'shapley_on_value']

def gwc_run(env=None, agent=None, states_to_explain=None):
    # ------------------------------------------------- TRAIN
    train(agent, env, 1e7)
    ...
    gamec.compute_v_Cs()
    characteristic_modes = gwc_get_characteristic_modes()
